 ...
 */
public class KenBurnsView2 extends FrameLayout {
    ...
    public KenBurnsView2(Context context) {
        this(context, null);
    }

    public KenBurnsView2(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public KenBurnsView2(Context context, AttributeSet attrs, int defStyle) {
...
//        Log.d(TAG, "starting Ken Burns animation " + propertyAnimator);
    ...
    public void setResourceBitmap(Bitmap... bitmaps) {
        mBitmap = bitmaps;
        for (int i = 0; i < mImageViews.length; i++) {
    ...
    public void setResourceUrl(String imageLink, boolean doBlur) {


        if (doBlur) {
            Glide.with(getContext()).load(imageLink).centerCrop().diskCacheStrategy(DiskCacheStrategy.ALL).bitmapTransform(new BlurTransformation(getContext(), 25)).into(mImageViews[0]);
            Glide.with(getContext()).load(imageLink).centerCrop().diskCacheStrategy(DiskCacheStrategy.ALL).bitmapTransform(new BlurTransformation(getContext(), 25)).into(mImageViews[1]);
        } else {
