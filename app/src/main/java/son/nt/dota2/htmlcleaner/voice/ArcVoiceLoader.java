...
public abstract class ArcVoiceLoader extends ContentLoader<List<SpeakDto>> {
    ...
    public ArcVoiceLoader(HttpUriRequest httpRequest, boolean isCache) {
    ...
    private List<SpeakDto> getVoiceWithImage(TagNode nodeA, String group) {
        ...
        try {
            ...
            for (int i = 0; i < objB.length; i++) {
                ...
                speakDto.no = i;
                list.add(speakDto);
                String t = "During earlygame, within 1000 radius";
            }
    ...
    private List<SpeakDto> getMeeting(TagNode nodeA) {
        List<TagNode> tagNodes = nodeA.getChildTagList();
        int count = 1;
        String group = "Meeting an ally";
        TagNode nodeKilling = null;
        for (int i = 0; i < tagNodes.size(); i++) {

            TagNode tag = tagNodes.get(i);
            try {
                if (tag.getText().toString().contains("During earlygame")) {
                    Logger.debug(TAG, ">>>" + "During earlygame");
                    nodeKilling = tagNodes.get(i + 1);
                    break;
                }
            } catch (Exception e) {
                e.printStackTrace();
        ...
        List<SpeakDto> listVoiceImage = getVoiceWithImage(nodeKilling, group);
        for (SpeakDto d : listVoiceImage) {
            Logger.debug(TAG, ">>>" + "image:" + d.rivalImage);
        }

        return listVoiceImage;
    }

    private List<SpeakDto> getKillingSpecial(TagNode nodeA) {
        List<TagNode> tagNodes = nodeA.getChildTagList();
        //special hero killing
        String group = "Killing a specific enemy";
        TagNode nodeKilling = null;
        for (int i = 0; i < tagNodes.size(); i++) {
            ...
            try {
                if (tag.getText().toString().contains("Killing a specific enemy")) {
                    Logger.debug(TAG, ">>>" + "Killing a specific enemy.");
                    nodeKilling = tagNodes.get(i + 1);
                    break;
                }
            } catch (Exception e) {
        ...
        List<SpeakDto> listKill = getVoiceWithImage(nodeKilling, group);
        for (SpeakDto d : listKill) {
            Logger.debug(TAG, ">>>" + "link:" + d.rivalImage);
        }
        return listKill;
    }
